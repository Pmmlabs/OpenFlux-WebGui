...
доработками, взятыми из другого форка сообщества —
[danusha2345/OpenFlux](https://github.com/danusha2345/OpenFlux).
Что добавлено/изменено относительно апстрима:

- **Мультиклиентская выходная нода + локальная веб-панель управления**
  (`--role=exit-panel`) — несколько клиентов в одном процессе, у каждого свой
  Yandex.Docs / Volga / MAX / Cups.online / Mail.ru документ или комната;
  веб-интерфейс с логином позволяет добавлять и удалять их «на лету», без
  перезапуска. Все клиенты используют общий Noise-идентификатор панели;
  файл PSK у конкретного клиента опционально закрывает именно его от
  посторонних. См.
...
- **Шифрование Noise NKpsk0** (X25519 + AES-256-GCM, ротация сессионных
  ...
  включено по умолчанию, если не передан `--allow-plaintext`.
- **Multi-stream транспорт** — `--url` принимает список документов через
  запятую (yandex/vyandex), соединения распределяются между ними с
  переключением на остальные при обрыве одного.
...
- **`netguard`** — деннилист SSRF: по умолчанию выходная нода блокирует
  ...
  отключает).
- **~20 фиксов корректности/надёжности/безопасности** на стороне выходной
  ноды и транспортов, каждый с регрессионным тестом: две паники, вызываемые
  удалённой стороной, decompression-bomb DoS, тихо игнорируемый `--local-ip`,
  RST от клиента, не доходящий до реального сервера, утечка памяти/горутин на
  зависших L4-реле, тихо проглатываемые ошибки отправки транспорта,
  блокировка всей обработки пакетов на время periodic-скана conntrack,
  отсутствие graceful shutdown по SIGTERM, лимит записей conntrack,
  half-close на всех путях данных вместо жёсткого закрытия, соответствие
  SOCKS5 RFC 1928 (IPv6, коды ошибок, точное чтение), фиксы
  реконнекта/keepalive в транспортах Yandex/Volga/MAX/Mail.ru.
- **iOS-мост не поддерживается в этом форке** — `export_ios.go` собирается
  под `//go:build ios` (унаследован от апстрима/форка сообщества), но
  iOS-приложение и CI для него здесь не публикуются.
- `build_android.sh` теперь учитывает ОС хоста (macOS/Linux/Windows) и
  собирает все три ABI, под которые
  [форк Android-приложения](https://github.com/Kingofthedivanich/OpenFluxAndroid)
  реально поставляет `jniLibs`, а не только `arm64-v8a` с хоста на macOS.
...
адресу `--panel-addr` (добавление/удаление, статус и статистика) без
перезапуска; регистрации сохраняются в `--panel-data` и подхватываются при
следующем запуске. `--panel-addr` должен оставаться на `127.0.0.1`;
подключайтесь удалённо через SSH-туннель (`ssh -L 8088:127.0.0.1:8088
user@host`), а не открывайте наружу напрямую. `l3` в этой роли намеренно не
предлагается — см. обычный `--role=exit` выше, если нужен сырой SNAT/DNAT.
