...
func TestUpstreamSOCKS5Integration(t *testing.T) {
	...
	tunExit := mustNewTCPTunnel(t, transExit, true, socksAddr)
	if tunExit == nil {
		t.Fatalf("Failed to create exit node tunnel")
	}

	// 5. Start Client tunnel
	tunClient := mustNewTCPTunnel(t, transClient, false, "")
...
func TestFullSocks5Chain(t *testing.T) {
	...
	// 4. Exit node
	tunExit := mustNewTCPTunnel(t, transExit, true, upstreamSocksAddr)
	if tunExit == nil {
...
func TestUpstreamFailureRejection(t *testing.T) {
	...
	tunExit := mustNewTCPTunnel(t, transExit, true, socksListener.Addr().String())
	if tunExit == nil {
		t.Fatalf("Exit node nil")
	}
	tunClient := mustNewTCPTunnel(t, transClient, false, "")
...
func mustNewTCPTunnel(t *testing.T, trans transport.Transport, isExitNode bool, upstreamProxy string) *TCPTunnel {
	t.Helper()
	tun, err := NewTCPTunnel(trans, isExitNode, upstreamProxy)
	if err != nil {
